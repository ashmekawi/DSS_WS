...
Public Class SameCompany
    ...
    End Function
    <WebMethod()>
    Public Function DamgUpdateDone(ByVal Done As String, ByVal CRANum As Integer) As Int32
        Dim str As String
        Dim x As New DbClass
        Dim con As String = ConfigurationManager.ConnectionStrings("DQ").ConnectionString
        str = "  update [SameCompanyDuplicate] set Done = '" & Done & "' where [FK_COMPANYCRA_NUM] ='" & CRANum & "' "
    ...
    End Function
    <WebMethod()>
    Public Function DamgData(ByVal CRANum As Integer) As DataTable
        Dim str As String
        Dim x As New DbClass
        Dim con As String = ConfigurationManager.ConnectionStrings("CRA00").ConnectionString
        str = "select CSO,NAME0,ID_NUMBER,sex,FK_POLICE_STATIFK,[BIRTH_DATE],OLD_BIRTH_DATE,OLD_ID_NUMBER,cname  from SameCompanyDuplicate where [FK_COMPANYCRA_NUM] ='" & CRANum & "'  and CSO not in (select cso from [DQ].dbo.tempcso) order by cname"
        Return x.FillDataTable(str, con, "BRN")
    End Function
    <WebMethod()>
